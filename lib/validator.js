...
function bindEvt(input, field, tag){
    ...
    !tag.formInputListenersMap[id] && (tag.formInputListenersMap[id] = []);
    tag.formInputListenersMap[id].push(validateHandler);
}
...
function unbindEvt(tag){
    ...
    for(var input of tag.formInputListenersMap.keys()){
        var listeners = tag.formInputListenersMap[input.dataset.eid];
...
function initForm(){
    ...
    this.on('mount', function(){
        walkTheDOM(me.root, function (node) {
            if (node.nodeType === 1) {
